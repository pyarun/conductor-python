...
async def main():
    ...
    async with ApiClient(api_config) as api_client:
        ...
        await orchestrator.add_prompt_template(
            moderator, moderator_text, "moderator instructions"
        )
        await orchestrator.associate_prompt_template(
            moderator, moderator_provider, [moderator_model]
        )

        await orchestrator.add_prompt_template(
            agent1, agent1_text, "agent1 instructions"
        )
        await orchestrator.associate_prompt_template(
            agent1, agent1_provider, [agent1_model]
        )

        await orchestrator.add_prompt_template(
            agent2, agent2_text, "agent2 instructions"
        )
        await orchestrator.associate_prompt_template(
            agent2, agent2_provider, [agent2_model]
        )

        get_context = SimpleTask(
            task_reference_name="get_document", task_def_name="GET_DOCUMENT"
        )
        ...
        moderator_task = LlmChatComplete(
            ...
            max_tokens=2000,
            llm_provider=moderator_provider,
        ...
        agent1_task = LlmChatComplete(
            ...
            llm_provider=agent1_provider,
            model=agent1_model,
            instructions_template=agent1,
            messages=[
                ChatMessage(role="user", message=moderator_task.output("result"))
            ],
            template_variables=template_vars,
        )

        set_variable1 = (
            SetVariableTask(task_ref_name="task_ref_name1")
            .input_parameter(
                "history",
                [
                    ChatMessage(
                        role="assistant", message=moderator_task.output("result")
                    ),
                    ChatMessage(
                        role="user",
                        message="["
                        + "${workflow.input.ua1}] "
                        + f'{agent1_task.output("result")}',
                    ),
                ],
            )
        ...
        set_variable2 = (
            SetVariableTask(task_ref_name="task_ref_name2")
            .input_parameter(
                "history",
                [
                    ChatMessage(
        ...
        wf = AsyncConductorWorkflow(
            name="multiparty_chat_tmp", version=1, executor=workflow_executor
        )
        ...
        """
        next_up = SwitchTask(
            task_ref_name="next_up_ref", case_expression=script, use_javascript=True
        )
        ...
        wf.timeout_seconds(1200).timeout_policy(
            timeout_policy=TimeoutPolicy.TIME_OUT_WORKFLOW
        )
        await wf.register(overwrite=True)

        result = await wf.execute(
            wait_until_task_ref=agent1_task.task_reference_name,
            wait_for_seconds=1,
            workflow_input=wf_input,
        )

        result = await workflow_client.get_workflow_status(
            result.workflow_id, include_output=True, include_variables=True
        )
        print(f"started workflow {api_config.ui_host}/execution/{result.workflow_id}")
        while result.status == "RUNNING":
            await asyncio.sleep(10)  # wait for 10 seconds LLMs are slow!
            result = await workflow_client.get_workflow_status(
                result.workflow_id, include_output=True, include_variables=True
            )
